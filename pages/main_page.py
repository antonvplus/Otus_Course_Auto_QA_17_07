...
class MainPage(BasePage):
    ...
    def check_item_in_cart(self) -> None:
        ...
        assert self.is_element_clickable(CartPageLocators.PROCEED_TO_CHECKOUT_BUTTON), "The product was not added to the cart"
    ...
    def check_price(self, currency: str) -> None:
        if currency == 'EUR':
            ...
            assert self.get_text_element(MainPageLocators.PRICE_FIRST_PRODUCT) == '$21.86', \
                f"Expect: '$21.86', Actual: '{self.get_text_element(MainPageLocators.PRICE_FIRST_PRODUCT)}'"
            assert self.get_text_element(MainPageLocators.PRICE_SECOND_PRODUCT) == '$32.83', \
                f"Expect: '$32.83', Actual: '{self.get_text_element(MainPageLocators.PRICE_SECOND_PRODUCT)}'"
            assert self.get_text_element(MainPageLocators.PRICE_THIRD_PRODUCT) == '$33.15', \
